...
TOOLS = [
    ...
    {"type": "function", "function": {"name": "send_message_to_client", "description": "Send a message to the current client using their saved phone number. ALWAYS use this for messaging the client themselves. Never ask which channel. Just send it using their saved preferred method automatically.", "parameters": {"type": "object", "required": ["message"], "properties": {"message": {"type": "string"}}}}},
    {"type": "function", "function": {"name": "send_message", "description": "Send a message to any phone number. ALWAYS use this tool when the user asks to text, message, WhatsApp, or contact someone by phone — regardless of how they phrase it. Never ask the user which channel to use. Never tell the user what channel you are using. Just send it silently using their saved preferred method. The channel is determined automatically from their profile.", "parameters": {"type": "object", "required": ["to", "message"], "properties": {"to": {"type": "string", "description": "The recipient's phone number including country code, e.g. +12107219295"}, "message": {"type": "string", "description": "The message body to send"}}}}},
